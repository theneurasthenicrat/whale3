...
//	      throw new DataBaseException("Database error while trying to create a new poll. Error information: candidate #" + nb + " -> " + result2)
  ...
  private def insertPreferenceModel(preferenceModelId: String): Unit = {
    ...
    try {
      ...
      val result: ResultSet = stmt.executeQuery("SELECT * FROM PreferenceModels WHERE preferencemodelid = '" + preferenceModelId + "';")
      if (!result.next) {
  ...
  private def createPoll(pollTitle: String, pollDescription: String, creationDate: Date, closingDate: Date, pollType: Int, preferenceModelId: String, owner: User, candidateLabels: List[String], attributes: Map[String, String]): Poll = {
    ...
    try {
      val connec: java.sql.Connection = Connection.getConnection()
      stmt = connec.createStatement()    
      val result: ResultSet = stmt.executeQuery("SELECT nextval('pollids');")
      result.next()
  ...
  */
  ...
  def getPollById(pollId: String): Poll = {
	...
	case None => throw new InvalidPreferenceModelException(result.getString(7))
      ...
      var poll: Poll = null
      pollType match {
	case 0 => poll = new Poll(result.getString(1), result.getString(2), result.getString(3), new java.util.Date(result.getDate(4).getTime()), new java.util.Date(result.getDate(5).getTime()), preferenceModel, owner, candidates, votes, attributes)
	case 1 => poll = new DatePoll(result.getString(1), result.getString(2), result.getString(3), new java.util.Date(result.getDate(4).getTime()), new java.util.Date(result.getDate(5).getTime()), preferenceModel, owner, candidates.asInstanceOf[List[TimeSlot]], votes, attributes)
