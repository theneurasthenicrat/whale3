...
class VoteController extends PollController with InvitationRequired {
  override def main(): Unit = {
	...
	return
      ...
      case VOTED_TWICE => error(getMessage("messages.poll", "votedTwice", Nil))
      case UNDEFINED_POLL => error(getMessage("messages.poll", "unspecifiedPoll", Nil))
      case POLL_CLOSED => error(getMessage("messages.poll", "isAlreadyClosed", ((new SimpleDateFormat("EEEE d MMMM yyyy", locale)).format(poll.closingDate)) :: ((new SimpleDateFormat("hh:mm", locale)).format(poll.closingDate)) :: Nil)); request.getRequestDispatcher("views/pollMenu.jsp").include(request, response); return
...
//    request.getRequestDispatcher("views/pollMenu.jsp").include(request, response)    
  ...
  protected def printRankingPollForm(poll: Poll, currentVotes: Array[String], tiesAllowed: Boolean): Unit = {
    ...
    //inputs of the form
    for (c <- poll.candidates) {
      out.println("<input type=\"text\" id=\"option" + c.candidatePosition + "\" name=\"candidate" + c.candidatePosition + "\" value=\"" + (if (currentVotes != null) currentVotes(c.candidatePosition) else "") + "\" />")
    }
    out.println("<div>")
    out.println("<input type=\"hidden\" name=\"nvid\" value=\"" + whale3.utils.Maths.generateRandomString(8) + "\"/>")
    out.println("<input type=\"submit\" value=\"OK\"/>")
